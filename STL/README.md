序列容器：   <br>
array：静态数组，大小固定、无法动态扩散或收缩。随机读改 O(1)。   <br>
* 底层是数组，元素都存储在连续的内存空间中。   <br>
* #include <array>   <br>
* std::array<int, 10> values {};   <br>

vector：动态数组，动态调整所占用的内存空间。随机读改 O(1)，在尾部插入/删除，O(1)；在头部或中部插入/删除，O(n)。   <br>
...
map：键是唯一的，不能重复。元素根据元素键的大小做升序排序（std::less<T>）。键值对一旦被存入容器中，键不能重复也不能修改。插入、删除、查找 O(log2n)。   <br>
* #include <map>   <br>
* std::map<std::string, int> myMap{{"myMap1", 10}, {"myMap2", 20}};   <br>

multimap：与map相同，但是键可以重复。插入、删除、查找 O(log2n)。   <br>
* #include <map>   <br>
* std::multimap<std::string, int> myMultimap{{"myMultimap1", 10}, {"myMultimap2", 20}};   <br>

set：各个元素键和值完全相同，各个元素的值不能重复，不能修改元素的值，元素根据元素键的大小做升序排序（std::less<T>）。只能用迭代器访问。插入、删除、查找 O(log2n)。   <br>
* #include <set>   <br>
* std::set<std::string> mySet{"myset1", "myset2"};   <br>

multiset：与set相同，但是值可以重复。插入、删除、查找 O(log2n)。   <br>
* #include <set>   <br>
...
unordered_map：无序，各个元素的键不能重复也不能修改。插入、删除、查找 O(1)，最差 O(n)。   <br>
...
unordered_multimap：与unordered_map相同，但是允许存储多个键相同的键值对。插入、删除、查找 O(1)，最差 O(n)。   <br>
* #include <unordered_map>   <br>
* std::unordered_multimap<std::string, int> ummap{{"ummap1", 10}, {"ummap2", 20}};   <br>

unordered_set：无序，各个元素键和值完全相同，各个元素的值不能重复，不能修改元素的值。插入、删除、查找 O(1)，最差 O(n)。   <br>
* #include <unordered_set>   <br>
* std::unordered_set<std::string> uset{"uset1", "uset2"};   <br>

unordered_multiset：与unordered_set相同，允许存储值相同的元素，且这些元素会存储到哈希表中同一个桶(本质是链表)上。插入、删除、查找 O(1)，最差 O(n)。   <br>
* #include <unordered_set>   <br>
...
stack<T>：封装了deque<T>容器的适配器类模板，后入先出。顶部插入，顶部删除 O(1)。   <br>
* [满足条件的基础容器有 vector、deque、list]   <br>
* #include <stack>   <br>
* std::stack<int> values{1, 2, 3};   <br>
* std:stack<int, std::list<int>> mystack;   <br>

queue<T>：封装了deque<T>容器的适配器类模板，先入先出。尾部插入，头部删除 O(1)。   <br>
* [满足条件的基础容器有 deque、list]   <br>
* #include <queue>   <br>
* std::queue<int> values{1, 2, 3};   <br>
* std::queue<int, std::list<int>> myqueue;   <br>

priority_queue<T>：封装了vector<T>容器的适配器类模板，默认实现是一个会对元素排序，保证最大元素总在队列最前面的队列。从一端进(队尾)，从另一端出(队头)，且每次只能访问位于队头的元素。在创建时制定了一种排序规则（std::less<T>），优先级最大的元素最先出队列。插入、删除 O(log2n)。   <br>
